...
export class AdminUserController {
    async listAdmins(req, res) {
        ...
        try {
            ...
            const [admins] = await connection.query('SELECT id, nome, email, role, active, cpf_cnpj, endereco FROM admin');
            console.log(`Fetched ${admins.length} admins`);
            console.log('Fetching responsaveis from DB...');
            const [responsaveis] = await connection.query('SELECT id, nome, email, "cliente" as role, active FROM responsavel');
    ...
    async createAdmin(req, res) {
        ...
        try {
            ...
            const [existing] = await connection.query('SELECT id FROM admin WHERE email = ?', [email]);
            ...
            const [result] = await connection.query(query, [nome, email, hashedPassword, role || 'admin']);
    ...
    async updateAdmin(req, res) {
        ...
        const type = req.query.type; // 'admin' or 'responsavel'
        if (role === 'master') {
        ...
        try {
            ...
            const [currentUser] = await connection.query(`SELECT * FROM ${table} WHERE id = ?`, [id]);
    ...
    async deleteAdmin(req, res) {
        ...
        try {
            const table = type === 'responsavel' ? 'responsavel' : 'admin';
            const [user] = await connection.query(`SELECT * FROM ${table} WHERE id = ?`, [id]);
            if (user.length === 0) {
                return res.status(404).json({ error: 'Usuário não encontrado.' });
            }
