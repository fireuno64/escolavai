...
export class ContratoService {
    ...
    async createNewContract(data) {
        ...
        if (contratoAtivo) {
            ...
            await repository.updateStatus(contratoAtivo.id, 'CANCELADO', data.motivoCancelamento || 'Substituído por novo contrato');
        ...
        await pagamentoService.generatePaymentsForContract(novoContrato.id, data.criancaId, data.responsavelId, dataInicio, data.valorAnual, data.adminId);
        return novoContrato;
    }
    /**
    ...
    async renewContract(data) {
        ...
        await pagamentoService.generatePaymentsForContract(novoContrato.id, contratoVencido.criancaId, contratoVencido.responsavelId, dataInicio, novoValorAnual, contratoVencido.adminId);
        return novoContrato;
    }
    /**
     * Buscar contrato ativo de uma criança
     */
    async getActiveContract(criancaId) {
        return repository.findActiveByCriancaId(criancaId);
    }
    /**
     * Buscar histórico de contratos de uma criança
     */
    async getContractHistory(criancaId) {
        return repository.findByCriancaId(criancaId);
    }
    /**
     * Arquivar contratos vencidos automaticamente
     */
    async archiveExpiredContracts(adminId) {
     ...
     */
    async cancelContract(contratoId, motivo) {
     ...
     */
    async getArchivedContracts(adminId) {
