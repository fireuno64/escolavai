...
function numeroParaExtenso(valor: number): string {
    ...
    if (milhar > 0) {
        ...
        if (centena > 0 || dezena > 0 || unidade > 0) extenso += ' ';
    }
...
export class ContractController {
    ...
    async generateContract(req: Request, res: Response) {
        try {
            ...
            // Header
            doc.fontSize(10).font('Helvetica-Bold').text('FENATRESC', { align: 'left' });
            doc.fontSize(8).font('Helvetica').text('FEDERAÇÃO NACIONAL DE TRANSPORTE DE ESCOLARES', { align: 'left' });
            doc.moveDown(0.5);
            doc.fontSize(10).text(`Data Início: ${dataInicioFormatada}`, { align: 'right' });
            doc.moveDown(1.5);
            ...
            doc.font('Helvetica-Bold').text('ANDERSON DE ALMEIDA SHIOTOKO', { continued: true });
            doc.font('Helvetica').text(', com o endereço à Av. Laurita Ortega Mari, 1470 - Pq. Pinheiros, C.G.C/C.P.F. Nº 256.831.028-63, doravante denominado ', { continued: true });
            ...
            doc.fontSize(10).font('Helvetica-Bold').text('CLÁUSULA 1ª - ', { continued: true });
            ...
            doc.fontSize(9).text('Parágrafo primeiro - Na impossibilidade do aluno ficar na porta de sua residência, deverá aguardar o transporte em local apropriado, seguro e mais próximo de sua residência.');
            doc.text('Parágrafo segundo – Em hipótese alguma poderá o transportador esperar o aluno.');
            doc.moveDown(0.8);

            doc.fontSize(10).font('Helvetica-Bold').text('CLÁUSULA 6ª - ', { continued: true });
            ...
            doc.fontSize(9).text('Parágrafo único - não será permitido que o aluno use palavras de baixo calão, agressões a outro aluno ou atitudes imorais, que poderá ser repreendida de maneira educada e verbal pelo contratado, notificando o responsável de imediato.');
            ...
            doc.fontSize(10).font('Helvetica-Bold').text('CLÁUSULA 9ª - ', { continued: true });
            ...
            doc.fontSize(9).text('a) O cancelamento neste caso não implicará multa contratual para nenhuma das partes.');
            doc.moveDown(0.8);

            doc.fontSize(10).font('Helvetica-Bold').text('CLÁUSULA 10ª - ', { continued: true });
            ...
            doc.fontSize(9).text('Parágrafo primeiro – O valor da parcela poderá ser reajustado conforme o aumento dos combustíveis e seus derivados.');
            doc.moveDown(0.8);

            doc.fontSize(10).font('Helvetica-Bold').text('CLÁUSULA 11ª - ', { continued: true });
            ...
            doc.fontSize(9).text('a) Nos dias de Excursões / Passeios o transporte só será realizado em horário normal de aula.');
            doc.moveDown(2);
            ...
            doc.fontSize(10).text(`São Paulo, ${dia} de ${mesExtenso} de ${ano}.`, { align: 'center' });
            doc.moveDown(4);
            ...
            doc.text('Contratado', 350, doc.y - 12, { width: 150, align: 'center' });
